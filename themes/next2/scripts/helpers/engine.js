...
hexo.extend.helper.register("next_js", function (file, pjax = false) {
  ...
  const links = {
    ...
    mycdn: this.env.debug
      ? this.url_for(`${this.theme.js}/${file}${tail}`)
      : `${this.theme.mycdn || ""}${this.url_for(`${this.theme.js}/${file}${tail}`)}`,
  };
  const src = links[internal] || links.local;
  return `<script ${pjax ? "data-pjax " : ""}src="${src}"></script>`;
});
hexo.extend.helper.register("url_for2", function (file, path) {
  var temp = `${this.url_for(file)}${path}`;
  var last = temp + tail;
  if (this.env.debug) {
    return last;
  } else {
    return `${this.theme.mycdn || ""}${last}`;
  }
});
